...
def predict(opt, model, test_split):
    ...
    eeg_var= eeg_test.generate_artificial_batch([torch.tensor(1)])
    eeg_var_1= eeg_test.generate_artificial_batch([torch.tensor(2)])
    
    eeg_var = eeg_var.to(opt.device)
    audio_var = audio_var.to(opt.device)
    video_var = video_var.to(opt.device)
